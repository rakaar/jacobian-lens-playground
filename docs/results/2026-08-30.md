...
## Downstream J-Lens trajectories at P17, P19, and P21
...
Thus P17 and P19 are clean tests of cross-position propagation: neither
position is directly edited. P21 is different because it is itself the L22
spider-suppression site, although it receives no ant injection.

| Readout position | Prompt token | Direct SAE edit at this position? | L22 residual delta norm | L33 residual delta norm | L33 clean/edited cosine |
| --- | --- | --- | ---: | ---: | ---: |
| P17 | ` animal` | No | 1013.0 | 3423.8 | 0.9972 |
| P19 | `?` | No | 2721.9 | 15026.0 | 0.9944 |
| P21 | turn-boundary newline | Yes: L22 spider suppression only | 3707.4 | 16262.1 | 0.8705 |

All three figures come from the same frozen forward pass and therefore have
...
is more strongly associated with `Legs`, `Four`, and `Eight`. The readout does
not form a simple explicit `Six` trajectory at P19, but it clearly shifts the
quantity-related state before answer generation.
...
J-space concepts do change across token positions after the SAE intervention;
the network does not carry one fixed `ant` coordinate unchanged through depth.
P17 shows a small propagated categorical change, P19 shows a larger propagated
quantity change, and P21 shows the strongest category-to-count trajectory.
The strongest explicit `Six` readout occurring at P21 rather than P16 also
fits the fact that answer prediction is made from the later turn boundary.
...
- [Source script](../assets/results/2026-08-30/run_sae_jlens_p16_downstream.py)
- Figures: `docs/assets/results/2026-08-30/sae_jlens_p{17,19,21}_downstream_top5.png`
