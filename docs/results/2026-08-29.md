...
## Cumulative ant-layer factorial on the 95% base

The single-layer gains did **not** add together. This experiment froze the exact L7+L22 configuration at `P(Six) = 95.2221%`, then tested every on/off subset of the L24, L25, L26, and L27 ant features: 16 configurations including the no-add-on control. Every enabled add-on used natural-reference factor `1` at position 16 (`spinning`); no extra spider feature was suppressed in the add-on layers.

L24 F209073, L26 F73903, and L27 F248182 were the primary candidates because their normalized explicit-ant reference activations passed the `0.4` cutoff. L25 F236757 was naturally active on that reference but reached only `0.185`, so every configuration containing it is marked `L25*` and remains below-cutoff exploratory evidence.

| Add-on subset | P(Six) | P(Eight) | P(Four) | Observed log odds | Additive prediction | Residual |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| None: frozen L7+L22 base | 95.2221% | 4.7408% | 0.0319% | 3.00 | 3.00 | 0.00 |
| L24 | **96.2323%** | 3.7313% | 0.0323% | 3.25 | 3.25 | 0.00 |
| L26+L27 | **96.2323%** | 3.7313% | 0.0323% | 3.25 | 3.50 | -0.25 |
| L24+L26 | 95.2135% | 4.7404% | 0.0410% | 3.00 | 3.50 | -0.50 |
| L24+L26+L27 | 93.9363% | 6.0051% | 0.0520% | 2.75 | 3.75 | **-1.00** |
| L24+L25*+L26+L27 | 93.9365% | 6.0052% | 0.0520% | 2.75 | 4.00 | **-1.25** |

Each add-on singleton raised `log P(Six) - log P(Eight)` by approximately `0.25`. Independent addition would therefore predict log odds `3.75` for the three primary layers and `4.00` after also adding L25*. Instead, both cumulative configurations landed at `2.75`, below even the frozen base's `3.00`. This is active cancellation or redundancy at the measured output—not four independent votes for Six. None of the 16 subsets reached `99%`; the best result remained the L24 singleton at `96.2323%`.

![All 16 factor-one add-on subsets, plus observed versus additive-predicted Six/Eight log odds. Multi-layer points fall below the diagonal, with the full primary and four-layer interventions missing their additive predictions by 1.00 and 1.25 log-odds units. Purple configurations include the below-cutoff L25 feature.](../assets/results/2026-08-29/spider_ant_cumulative_factorial.png)

*The left panel reports actual next-token percentages. The right panel makes the interaction visible: every plotted multi-layer intervention falls below the independent-addition line.*

![Cumulative path from the frozen L7 plus L22 base through L24, L26, L27, and finally exploratory L25. P(Six) rises to 96.232 percent after L24, returns near the base after L26, then falls to 93.936 percent after L27 and remains there after L25.](../assets/results/2026-08-29/spider_ant_cumulative_path.png)

*Adding more calibrated layers progressively reverses the useful L24 gain along this path. `P(Four)` remains below `0.052%`, so the deterioration is still concentrated in the Six/Eight competition rather than broad output disruption.*

The clean spider baseline reproduced `P(Eight) = 99.99825%`. The all-candidate factor-`0` cell exactly matched the frozen base; all three above-cutoff singleton cells exactly matched the prior independent add-on run; and the pre/post clean baselines matched exactly, confirming hook cleanup.

Source: `scripts/run_spider_ant_cumulative_layer_sweep.py` · [download script snapshot](../assets/results/2026-08-29/run_spider_ant_cumulative_layer_sweep.py)

Figures: [factorial summary](../assets/results/2026-08-29/spider_ant_cumulative_factorial.png) · [cumulative path](../assets/results/2026-08-29/spider_ant_cumulative_path.png)

Data: [raw JSON](../assets/results/2026-08-29/spider_ant_cumulative_layer_sweep.json) · [tidy CSV](../assets/results/2026-08-29/spider_ant_cumulative_layer_sweep.csv) · [frozen feature manifest](../assets/results/2026-08-29/spider_ant_cumulative_layer_manifest.json) · [complete run log](../assets/results/2026-08-29/spider_ant_cumulative_layer_run.log)
